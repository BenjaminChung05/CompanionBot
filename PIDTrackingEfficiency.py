...
KD_DIST = 0.00001 
TARGET_AREA = 150000 
PERSON_NEAR_AREA = 125000
PERSON_NEAR_FRAMES = 3
PERSON_FAR_FRAMES = 4
...
def vision_loop():
    ...
    while True:
        ...
        if current_internal_id is not None and current_internal_id in track_ids:
            ...
            # Camera-based proximity lockout with hysteresis to avoid jitter.
            if current_area >= PERSON_NEAR_AREA:
                near_frame_count += 1
                far_frame_count = 0
            else:
                far_frame_count += 1
            ...
            if near_frame_count >= PERSON_NEAR_FRAMES:
                person_is_near = True
            elif far_frame_count >= PERSON_FAR_FRAMES:
                person_is_near = False
            ...
            if person_is_near:
                if not manual_override:
                    stop_motors()
                cv2.putText(frame, "PERSON NEAR (CAMERA): STOPPED.", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(frame, f"Area: {current_area}", (10, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)
            elif current_distance < STOPPING_DISTANCE_CM:
                if not manual_override:
                    stop_motors()
